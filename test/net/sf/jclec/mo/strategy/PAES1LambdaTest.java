...
/**
 * Tests for the (1+lambda)-PAES strategy.
 * 
 * <p>HISTORY:
 * <ul>
...
public class PAES1LambdaTest extends MOStrategyTest {
	 ...
	 * <code>lambda</code> value. If only one individual constitutes
	 * the population (<code>mu==1</code>), then each parent will
	 * be a copy of the current member of the population.
	 * */
	@Test
	public void testMatingSelectionParentsCreated(){
		...
		// Check that each parent is equal to the current solution
		for(int i=0; i<lambda; i++)
			assertTrue(parents.get(i).equals(population.get(0)));
	}

	/**
	 * Test that the environmental selection method returns
	 * the expected number of solutions (<code>mu==1</code>)
	 * and that solution is the non-dominated offspring. 
	 * */
	@Test
