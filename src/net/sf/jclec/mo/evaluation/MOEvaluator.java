...
/**
 * Evaluator for a multi-objective problem. It evaluates each solution using a
 * list of configured objectives. Solutions are evaluated sequentially.
 * 
 * <p>HISTORY:
 * <ul>
...
public class MOEvaluator extends AbstractEvaluator implements IMOEvaluator {
	...
	/** Serial ID */
	private static final long serialVersionUID = 1256059102356112801L;
	 ...
	 * */
	public MOEvaluator(List<Objective> objectives){
		super();
		int size = objectives.size();
		this.objectives = new ArrayList<Objective>(size);
		for(int i=0; i<size; i++){
			this.objectives.add(objectives.get(i));
		}
	}
	...
	/**
	 * {@inheritDoc}
	 * <p>Parameters for <code>MOEvaluator</code>
	 * are:
	 * <ul>
	 * <li>objectives (List of <code>Objective</code>):
	 * The list of objectives to be evaluated. Parameters 
	 * for every objective (using the tag "objective") are:
	 * <ul>
	 * <li><code>[@type] (String)</code>: 
	 * Path to class that implements the objective.</li>
	 * 
	 ...
	 * The objective should be maximized (true) or minimized (false).</li>
	 * </ul>
	 * 
	 * </li>
	 * </ul>
