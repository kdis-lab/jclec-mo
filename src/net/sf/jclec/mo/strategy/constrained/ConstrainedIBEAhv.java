...
import net.sf.jclec.mo.strategy.IBEAhv;

/**
 * An IBEAhv strategy for problems with constraints. It sets the worst fitness values
 ...
 * @see IConstrained
 * */
public class ConstrainedIBEAhv extends IBEAhv {
	...
	/** Serial ID */
	private static final long serialVersionUID = 1859528698851709683L;
	 ...
	 * Empty constructor.
	 * */
	public ConstrainedIBEAhv(){
		super();
	}

	/**
	 * {@inheritDoc}
	 * <p>This version of IBEAhv requires the configuration of
	 * a comparator for constrained problems.
	 * */
	@Override
	...
	protected void fitnessAssignment(List<IIndividual> population, List<IIndividual> archive) {
		...
		// Compute objective bounds and fitness components considering the scale objective space
		computeBounds(copy);
	...
	public List<IIndividual> environmentalSelection(List<IIndividual> population, 
		...
		while(n<nRemovals){
			...
			for(int i=0; i<size; i++){
				...
				else{ // This seems to be redundant
					if(isMaximized())
						fvalue = Double.MIN_VALUE;
					else
						fvalue = Double.MAX_VALUE;
