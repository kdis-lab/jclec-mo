...
/**
 * F2 function of ZDT4 ptimization problem. For a definition of the ZDT4 problem, see:
 *  
 ...
 * 
 * <p>URL: {@link http://people.ee.ethz.ch/~sop/download/supplementary/testproblems/zdt4/}</p>
 * 
 * <p>HISTORY:
 * <ul>
 ...
 * @see Objective
 * */
public class ZDT4F2Objective extends Objective {
	...
	public IFitness evaluate(IIndividual solution) {
		...
		if(solution instanceof RealArrayIndividual){
			...
			f1 = genotype[0];
			for(int i=1; i<m; i++){
				g += (genotype[i]*genotype[i] - 10*Math.cos(4*Math.PI*genotype[i]));
			}
			g = 1 + 10*(m-1) + g;
			h = 1 - Math.sqrt(f1/g);
