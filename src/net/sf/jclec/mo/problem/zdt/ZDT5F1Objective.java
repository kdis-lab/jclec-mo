...
/**
 * F1 function of ZDT5 optimization problem. For a definition of the ZDT5 problem, see:
 *  
 ...
 * @see Objective
 * */
public class ZDT5F1Objective extends Objective {
	...
	public IFitness evaluate(IIndividual solution) {
		...
		if(solution instanceof BinArrayIndividual){
			...
			// Get the 'u' value (number of ones in the first variable)
			for(int i=0; i<30; i++){
				if(genotype[i]==1)
					ones++;
			}
			fitness = new SimpleValueFitness(1+ones);
		}
		return fitness;
	}
}
